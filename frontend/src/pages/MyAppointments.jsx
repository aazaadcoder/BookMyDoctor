...
const MyAppointments = () => {
  const { backendUrl, token, appointments, setAppointments, getDoctorsData } =
        ...
        {appointments.map((item, index) => (
          ...
          >
            ...
            <div className="flex flex-col justify-end gap-2">
              ...
              {item.cancelled && !item.isCompleted && (
                <button className="text-sm sm:min-w-48 py-2 border rounded border-red-500 text-red-500 ">
                  Appointment Cancelled
                </button>
              )}

              {item.isCompleted && (
                <button className="text-sm sm:min-w-48 py-2 border rounded border-green-500 text-green-500 ">
                  Appointment Completed
                </button>
              )}

              {/* todo update the code here to reflect iscompleted and payment */}
            </div>
          </div>
        ))}
      </div>
